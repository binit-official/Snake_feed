...
while running:
    if begin:
        ...
        snake_rect = pygame.rect.Rect([randrange(0, config.SCREEN_SIZE, config.GRID_CELL_SIZE),
                                       randrange(0, config.SCREEN_SIZE, config.GRID_CELL_SIZE), config.SNAKE_PART_SIZE,
    ...
    if snake_rect.left < 0 or snake_rect.right > config.SCREEN_SIZE or snake_rect.top < 0 or snake_rect.bottom > config.SCREEN_SIZE or len(
            snake_parts) != len(set(snake_part.center for snake_part in snake_parts)):
        begin = True
